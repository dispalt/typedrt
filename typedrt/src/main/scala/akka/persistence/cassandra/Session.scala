...
object Session {
  ...
  def apply[F[_]](datastaxSession: DatastaxSession)(implicit cs: ContextShift[F], F: Async[F]): Session[F] =
    new Session[F] {

      override final def execute(query: String): F[ResultSet] =
        cs.evalOn(immediateExecutionContext) {
          F.async { cb =>
