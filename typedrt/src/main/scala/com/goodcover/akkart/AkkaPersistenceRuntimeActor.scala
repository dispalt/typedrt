...
import com.goodcover.akkart.SnapshotPolicy.{EachNumberOfEvents, Never}
...
object SnapshotPolicy {
  ...
  private[akkart] case object Never extends SnapshotPolicy[Nothing]

  final private[akkart] case class EachNumberOfEvents[State: PersistCodec](
    numberOfEvents: Int,
    keepN: Int,
...
/**
  ...
  */
final class AkkaPersistenceRuntimeActor[M[_[_]], F[_], I: KeyCodec, State, Event] private[akkart] (
  entityName: String,
  actions: M[ActionT[F, I, State, Event, *]],
  initialState: I => State,
  updateState: (State, Event) => State,
  ...
  implicit val ec: monix.execution.Scheduler = monix.execution.Scheduler(context.dispatcher)
