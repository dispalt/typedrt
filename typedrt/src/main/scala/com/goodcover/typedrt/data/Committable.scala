...
final case class Committable[F[_], +A](commit: F[Unit], value: A) {
  ...
  def mapF[I[_]](f: F[Unit] => I[Unit]): Committable[I, A] = {
    copy(commit = f(commit))
  }
...
object Committable {
  ...
  implicit def catsMonadAndTraversInstance[
    F[_]: Applicative
    ...
    new Monad[Committable[F, ?]] with Traverse[Committable[F, ?]] {
      ...
      override def flatMap[A, B](fa: Committable[F, A])(f: (A) => Committable[F, B]): Committable[F, B] =
        f(fa.value)

      @scala.annotation.tailrec
      override def tailRecM[A, B](a: A)(f: (A) => Committable[F, Either[A, B]]): Committable[F, B] = {
        val c = f(a)
        c.value match {
      ...
      override def foldRight[A, B](fa: Committable[F, A], lb: Eval[B])(f: (A, Eval[B]) => Eval[B]): Eval[B] =
        f(fa.value, lb)
  ...
  def collector[F[_], A, B](pf: PartialFunction[A, B]): PartialFunction[Committable[F, A], Committable[F, B]] = {
