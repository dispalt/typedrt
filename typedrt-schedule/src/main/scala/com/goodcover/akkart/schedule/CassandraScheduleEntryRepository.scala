...
import com.goodcover.akkart.schedule.CassandraScheduleEntryRepository.{Queries, TimeBucket}
import com.goodcover.akkart.schedule.ScheduleEntryRepository.ScheduleEntry
import monix.eval.Task

class CassandraScheduleEntryRepository[F[_]](
  ...
  private val preparedDeleteEntry   = cassandraSession.prepare(queries.deleteEntry)

  override def insertScheduleEntry(id: ScheduleBucketId, entryId: String, dueDate: LocalDateTime): F[Unit] =
  ...
  override def deleteScheduleEntry(id: ScheduleBucketId, entryId: String): F[Unit] = {
    F.fromFuture(preparedDeleteEntry)
      .map(
        _.bind()
          .setString("schedule_name", id.scheduleName)
          .setString("schedule_bucket", id.scheduleBucket)
...
object CassandraScheduleEntryRepository {
  ...
  def initAndRegister[F[_]](queries: Queries)(implicit F: Monad[F]): Session.Init[F] = {
    val createTableViews: Session.Init[F] = Kleisli { session =>
      for {
    ...
    createTableViews >> registerCodec
  }

  def registerCodec[F[_]](implicit F: Monad[F]): Session.Init[F] = Kleisli { session =>
    for {
